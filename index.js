...
async function insertItem(client) {
    db = client.db('inventory');
    items_coll = db.collection('items');
    
    add = await inquirer.prompt([
        {
            type: 'input',
            name: 'name',
            message: 'Name:'
...
async function update_sale(client) {
    ...
    console.log(await items_coll.find({ _id: new ObjectId(update.id) }));
}
